...
const Navbar = () => {
  ...
  return (
    ...
    <header className="w-full bg-background sticky top-0 z-50">
      ...
      {/* ── EXPANDED STATE: date bar + big logo + nav ── */}
      <div className={`transition-all duration-300 ease-in-out overflow-hidden ${scrolled ? "max-h-0 opacity-0" : "max-h-40 opacity-100"}`}>
      ...
      {/* ── COMPACT STATE: single slim bar (visible when scrolled) ── */}
      <div className={`transition-all duration-300 ease-in-out overflow-hidden border-b border-border ${scrolled ? "max-h-14 opacity-100" : "max-h-0 opacity-0"}`}>
        <div className="container max-w-[1400px] mx-auto px-4 md:px-6 flex items-center justify-between py-2.5">
          <Link to="/" className="font-headline text-lg font-black tracking-tight text-foreground">
            Shared Ground
          </Link>
    ...
    {/* Backdrop — closes mobile menu on tap outside */}
    {menuOpen && (
      <div
